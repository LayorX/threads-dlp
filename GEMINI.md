...
您可以透過下方的公開儀表板，查看此專案自動抓取並分析的熱門影片資料庫。

- **儀表板連結:** [https://101.zeabur.app/](https://101.zeabur.app/)
- **帳號:** `admin`
- **密碼:** `password!`
...
18. **上傳器韌性與資料一致性強化**
    *   **問題:** `uploader.py` 的原始設計是在單一影片上傳失敗時，就中止整個上傳任務 (`break`)，這使得系統在面對暫時性網路錯誤或單一損壞檔案時缺乏韌性。此外，當影片檔案遺失但資料庫紀錄仍存在時，程式只會不斷地報錯並跳過，造成日誌汙染與資料不一致。
    *   **分析:** 「快速失敗」策略雖然安全，但降低了自動化效率。而對於遺失的檔案，僅僅跳過並不能解決根本的資料不一致問題。
    *   **修正:** 我們採納了更具品味的「容錯」與「清理」策略：
        1.  **提升韌性:** 將上傳失敗時的 `break` 改為 `continue`，允許程式在記錄錯誤後，繼續處理佇列中的下一個影片。
        2.  **保持一致性:** 當偵測到影片檔案不存在時，程式現在會主動呼叫 `database.py` 中的新函式 `delete_video_record`，直接刪除對應的資料庫紀錄，從根本上解決資料不一致的問題。

19. **YouTube API 上傳配額超限處理**
    *   **問題:** 當 YouTube API 回傳「`Error 400: The user has exceeded the number of videos they may upload.`」錯誤時，原有的錯誤處理邏輯會繼續嘗試上傳其他影片，這會浪費資源並產生不必要的錯誤日誌，因為此錯誤代表當日配額已用盡，所有後續上傳都將失敗。
    *   **分析:** YouTube 的上傳配額超限是一個非暫時性、系統性的錯誤。在此情況下，繼續嘗試上傳是無效的。
    *   **修正:** 我們引入了 `YouTubeQuotaExceededError` 自訂異常。當 `uploader.py` 偵測到此特定錯誤訊息時，會拋出此異常，並在 `run_upload_task` 函式中捕捉，隨後立即中止 (`break`) 整個上傳任務。這確保了系統在面對此類不可恢復的錯誤時，能及時停止並避免資源浪費。
...
- [ ] **修復 `-s` (搜尋關鍵字) 功能**
    - **問題描述:** 目前使用 `-s <keyword>` 參數時，程式似乎沒有根據關鍵字正確篩選出相關的影片。抓取到的內容可能與未使用 `-s` 參數時相同，或是完全不相關。
    - **初步 Issue 分析:**
...
- [x] **資料庫路徑重構:** 為了適應雲端部署的永久性磁碟掛載，建立了 `db` 資料夾，並將所有資料庫檔案的路徑參考都更新至此，同時更新了說明文件。
- [x] **完成 Zeabur 平台部署:** 微調 `Dockerfile` 與 `Procfile` 設定，確保在 Zeabur 環境中所有服務都能穩定運行。
- [x] **設定環境變數:** 在 Zeabur 平台上安全地設定 `sessionid`、`YT_REQUEST`、`GEMINI_API_KEY` 等所有必要的環境變數。
- [x] **端對端測試 (雲端):** 在部署完成後，進行一次完整的端對端測試，驗證從爬取、下載、上傳到排程的整個流程在雲端環境中是否正常。

### Phase 5: 文件與優化 (進行中)

- [x] **文件優化:** 重構 `README.md` 與 `README.en.md` 以提升清晰度，並為本地開發新增了詳細的 Docker 部署指南。
- [x] **上傳器健壯性優化:** 增強了 `uploader.py` 的錯誤處理邏輯。現在，單一影片上傳失敗不會再中斷整個任務，而是會繼續處理下一個。同時，增加了在影片檔案遺失時自動清理對應資料庫紀錄的功能，確保了資料的長期一致性。
- [x] **YouTube API 上傳配額超限處理:** 實作了針對 YouTube API 上傳配額超限錯誤的專門處理邏輯。當偵測到此類錯誤時，上傳任務將立即中止，避免不必要的資源消耗。
